    ...
    def _analyze_operation(self, path: str, method: str, op1: Dict, op2: Dict):
        ...
        if 'requestBody' in op1 and 'requestBody' in op2:
            self._compare_request_body(path, method, op1['requestBody'], op2['requestBody'])
             ...
             self.issues.append(CompatibilityIssue(path, method, "Request Body", "-", "Missing", "Request body was removed in new spec."))
    ...
    def _compare_schemas(self, path: str, method: str, location: str, s1: Dict, s2: Dict, item_name_prefix: str, visited=None):
        ...
        for prop in all_props:
            ...
            if prop in props1 and prop not in props2:
                self.issues.append(CompatibilityIssue(path, method, location, prop_path, "Missing", "Property removed in new spec."))
        ...
        if 'items' in s1 and 'items' in s2:
             ...
             self.issues.append(CompatibilityIssue(path, method, location, f"{item_name_prefix}[]", "Missing", "Array item definition removed in new spec."))
        elif 'items' in s2:
